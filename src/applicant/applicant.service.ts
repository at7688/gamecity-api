...
export class ApplicantService {
  ...
  async autoVerify(promotion_id: string, applicant_id: string) {
    ...
    if (
      ...
      await this.depositVerify(promotion_id, applicant_id);
    } else if (promotion.type === PromotionType.WATER) {
      await this.waterVerify(promotion_id, applicant_id);
    }
  }

  async waterVerify(promotion_id: string, applicant_id: string) {
    const promotion = await this.prisma.promotion.findUnique({
      where: { id: promotion_id },
  ...
  async depositVerify(promotion_id: string, applicant_id: string) {
    const promotion = await this.prisma.promotion.findUnique({
      where: { id: promotion_id },
      include: { recharge_reward: true, game_water: true },
    });
    ...
    const pRecord = await this.prisma.paymentDepositRec.findFirst({
      where: {
        player_id: applicant.player_id,
        created_at: {
    ...
    const bRecord = await this.prisma.bankDepositRec.findFirst({
      where: {
        player_id: applicant.player_id,
        created_at: {
          gte: promotion.start_at ? promotion.start_at : undefined,
    ...
    const { reward_amount, reward_percent, rolling_type } =
